...
```
zotero-bot/
├── run.py                  # Main entry point (gene + topic pipelines)
├── inverse_bot.py          # Orphan paper detection (citation centrality scoring)
├── genebot/
│   ├── zotero_client.py    # Zotero group library upload + collection hierarchy (pyzotero); DedupBaseline value object + get_dedup_baseline()
│   ├── rejection_log.py    # Near-miss article logging
│   ├── identifiers.py      # Canonical PMID/DOI/PMCID extraction from Zotero item data (shared by zotero_client/pdf_helpers/dedup)
│   ├── runtime.py          # Shared entry-point bootstrap: zotero_credentials/make_zotero_client/make_openalex_client/configure_logging
│   └── jsonstore.py        # read_json/write_json primitives shared by the cache/ledger files
│   # NOTE: openalex.py + hgnc.py moved to bio_toolkit (Phase 8); imported from there now
├── fetch_pdfs.py           # Central dispatcher: fetch PDFs for items missing full text
├── pdf_helpers.py          # Shared Zotero-scan / DOI-PMCID extract / download / link helpers
├── pdf_strategies/         # One module per PDF source: unpaywall, pmc, + browser-driven
│   │                       #   elsevier/wiley/springer/nature/bmj (browser_session + browser_base)
├── export_group_pdfs.py    # Copy the group's stored PDFs to the shared corpus (read-only on Zotero)
├── dedup_library.py        # Deduplicate library by DOI/PMID (dry-run default; --apply to execute)
├── audit_bot.py            # Library audit routine helper: trash off-topic + rescue dismissed (symmetric 2-model gate: Haiku screener + Sonnet adjudicator must concur). One library fetch/sweep (dedup baseline cached in audit_work/); batches carry verdict_out; emits data/audit_feedback.json (per-gene trash/rescue tally for genes.yml tuning)
├── genes.yml               # Gene list + config (generated by build_genes_yml.py -- see Generators below)
├── topics.yml              # Topic categories + sub-topics + keywords (generated by build_topics_yml.py)
├── generate_test_data.py   # Synthetic near-miss + flagged-papers data for dashboard testing
├── tests/                  # Unit tests (stdlib unittest) for pure logic
├── site/
...
├── requirements.txt        # pyzotero, requests, pyyaml (pinned) -- NOT bio_toolkit (see Dependencies)
...
`gene_pipeline.yml` dispatch inputs: `genes` (string, specific symbols).
...
- Batch Zotero upload (50 items/request, 60s timeout). A *connect* timeout (request never reached the server) is retried; a *read*/other timeout (the items may already be created server-side) is NOT retried -- the batch is deferred to the next run to avoid duplicates, and post-upload verification reports the gap
...
## Generators (genes.yml / topics.yml)

`genes.yml` and `topics.yml` are committed here and consumed by `run.py`, but
their generators live **in the vault**, not in this repo (they scan vault gene
notes + taxonomy data, which is established-knowledge that belongs to the
vault):

```
Ophtalmogenetics/_assets/code/gene-generation/build_genes_yml.py
Ophtalmogenetics/_assets/code/gene-generation/build_topics_yml.py
```

Each auto-detects the vault root and writes its output back here by default
(`<vault>/../lab/zotero-bot/{genes,topics}.yml`); override the target with
`--output PATH`. Run them from the vault when the gene list or taxonomy changes,
then commit the regenerated YAML in this repo.
...
Unit tests (stdlib `unittest`, no pytest required) cover the pure-logic pieces
-- PMID/DOI extraction, RejectionLog cumulative merge, dedup canonical
selection, and re-search-date tracking:
```bash
python -m unittest discover -s tests -v
```
For the dashboard, generate synthetic data and serve the static site:
```bash
...
- `run.py` logging is configured inside `main()`, not at module level (safe to import as library); all four entry points (`run`/`inverse_bot`/`audit_bot`/`dedup_library`) bootstrap through `genebot.runtime` (`configure_logging`, `zotero_credentials`, `make_zotero_client`, `make_openalex_client`) -- one seam for credentials/clients/logging, so none read `os.environ` for Zotero creds directly
- `run.py` `screen_works(works, *, text_excl, existing_pmids, existing_dois, rejection_log, refs_out)` is the shared discovery-pass screening step (preserve refs -> `work_to_record` -> text filter -> library dedup) that `process_gene`/`process_topic_subtopic` call for every source (search/re-search/citation/recent); MeSH exclusion stays at each call site because its filtered works are reused as citation seeds
- `zotero_client.py` `get_dedup_baseline(*, include_trashed=True)` returns a `DedupBaseline` (active `pmid_to_key`/`doi_to_key` + `trashed_pmids`/`trashed_dois`, with `existing_pmids`/`existing_dois` properties bundling active+trashed) in one call -- replaces the old ordered `get_existing_items()` -> `get_existing_dois()` -> trash dance whose wrong order silently yielded an empty dedup half
- Identifier extraction (PMID/DOI/PMCID) lives in `genebot.identifiers`; `zotero_client._extract_*_from_data`, `pdf_helpers.extract_doi/extract_pmcid`, and `dedup_library` all delegate there (canonical DOI extractor checks DOI field -> extra -> url)
- Cache/ledger JSON I/O goes through `genebot.jsonstore.read_json`/`write_json`
- The OpenAlex + HGNC clients now live in `bio_toolkit` (`bio_toolkit.clients.openalex`, `bio_toolkit.clients.hgnc`), not in `genebot/`. The bot uses `OpenAlexClient` (public methods `extract_pmid()`, `filter_by_mesh()`, `resolve_openalex_ids_to_pmids()`, `work_to_record()`, `fetch_works_by_dois()`, `fetch_works_by_pmids()`, `expand_citations()`, ...) and `get_gene_aliases()` imported from there
...
- `rejection_log.py` `category`/`subcollection` are **parallel** comma-separated lists: the Nth category is the parent of the Nth subcollection. They must only ever be merged as pairs via `_context_pairs()` / `_write_context_pairs()` / `_merge_context()` - deduplicating and sorting the two fields independently (the old `_merge_csv_field`) desynchronised them, so a near-miss shared by a gene and a topic ended up claiming subcollection `ABCA4` under category `1 - Anatomie`. `_build_hierarchy()` and `run.py` `_first_context_pair()` (rescue queue) both zip the pair, so a desync leaked genes into topic branches of the dashboard *and* made `process_rescue_queue()` create real gene collections under topic categories in Zotero
- `repair_near_miss_context.py` repairs files written before that fix: it maps each subcollection to its true category from genes.yml/topics.yml, plus names learned from the file's own single-collection entries (covers genes dropped from genes.yml and the French collection names used in the live group), then rebuilds `hierarchy`/`stats`. Dry-run by default; `--apply` rewrites in place with a `.bak`. The authoritative `near_misses.json` lives on gh-pages, so run it against the fetched gh-pages copy
...
- `run.py` recent additions: `_track_additions()` and `_register_new_records()` both filter by the `added_indices` set returned from `add_papers()` (the positions in the input list Zotero actually created), so failed uploads and DOI-only records are handled consistently; `save_recent_additions()` merges with previous data and prunes to 8 weeks; both gene and topic pipelines pass `additions_tracker` list
...
- `run.py` citation-cache write: a rotation gene's `last_full_expanded` is merged into its existing cache entry (`setdefault("genes", {}).setdefault(symbol, {})`), NOT assigned over it, so `last_search_date` survives and periodic re-search keeps working
- `zotero_client.py` `get_existing_items()` raises `RuntimeError` if the dedup baseline fetch fails after retries (instead of returning `{}`); `run.py main()` catches it and aborts the run, so a transient Zotero outage at startup can no longer wipe the dedup baseline and re-upload the whole library as duplicates
- `zotero_client.py` `add_relations_batch(edges)` replaces the old per-edge `add_relations`: it builds a symmetric adjacency from `{item_key: {related_keys}}` and fetches/patches each affected item at most once, throttled by `self.delay`
- `dedup_library.py` apply phase re-fetches each item's current version before update/delete (avoids HTTP 412 on stale versions) and deletes a duplicate only after its children are confirmed fetched and fully reparented (a parent delete trashes its children, so a failed reparent blocks the delete); credentials come from `bio_toolkit.config` like the rest of the project

## Dependencies

Python 3.10+. Install: `pip install -r requirements.txt`

This bot also imports **`bio_toolkit`**, the shared lab toolkit at `lab/tools/`
(its own repo; the importable package name is `bio_toolkit`, mapped from the
`tools/` dir via its `pyproject.toml`). It provides the OpenAlex + HGNC clients,
config (`ZOTERO_GROUP_ID`, `zotero_api_key()`), and PDF/cache helpers
(`pdf_corpus_dir()`). The shared source is published at
https://github.com/Hippolyte-Menou/bio_toolkit. For local dev, install it
editable: `pip install -e ../tools`. CI installs it via a pinned `git+https`
line in each workflow's "Install dependencies" step (bump that SHA -- or switch
to `@master` -- when the toolkit changes). It is **not** on PyPI and
intentionally **not** in `requirements.txt`, so a local
`pip install -r requirements.txt` won't clobber an editable install.
