...
Preserves existing collections, default exclusions, and per-gene
exclude_mesh/exclude_text overrides from the current genes.yml.
...
def build_override_index(existing_config: dict) -> dict[str, dict]:
    ...
    for gene in existing_config.get("genes", []):
        ...
        entry: dict = {}
        if "exclude_mesh" in gene:
            entry["exclude_mesh"] = gene["exclude_mesh"]
...
def build_genes_yml(vault_root: Path, genes_yml_path: Path) -> str:
    ...
    for entry in gene_entries:
        ...
        if "exclude_mesh" in entry:
            gene_block["exclude_mesh"] = entry["exclude_mesh"]
    ...
    header = (
        ...
        "# Per-gene exclude_mesh/exclude_text overrides are preserved across rebuilds.\n"
        ...
        "#   - Publication type is auto-derived from PubMed PT field\n"
